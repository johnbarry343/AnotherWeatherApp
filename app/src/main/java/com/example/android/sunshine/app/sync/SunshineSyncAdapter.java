...
public class SunshineSyncAdapter extends AbstractThreadedSyncAdapter
{
                                        ...
                                        String locationSetting) throws JSONException
    {
        ...
        try
        {
            ...
            for (int i = 0; i < weatherArray.length(); i++)
            {
                ...
                weatherValues
                        .put(WeatherContract.WeatherEntry.COLUMN_WEATHER_CONDITION_ID, weatherId);
            ...
            //            String[] resultStrs = cVVector.toArray(new String[cVVector.size()]);
            //            return resultStrs;
    ...
    long addLocation(String locationSetting, String cityName, double lat, double lon)
    {
        ...
        {
            locationId = locationCursor.getString(0);
        }
        else
        {
            Uri locationUri = WeatherContract.BASE_CONTENT_URI.buildUpon()
                    .appendPath(WeatherContract.PATH_LOCATION).build();
            ContentValues cv = new ContentValues();
            cv.put("city_name", cityName);
            cv.put("coord_lat", lat);
            cv.put("coord_long", lon);
            cv.put("location_setting", locationSetting);
            Uri returnedUri = mContext.getContentResolver().insert(locationUri, cv);
            locationId = returnedUri.getPathSegments().get(1);
        }
    ...
    public static void syncImmediately(Context context)
    {
        ...
        ContentResolver
                .requestSync(getSyncAccount(context), context.getString(R.string.content_authority),
                        bundle);
    }
        ...
        if (System.currentTimeMillis() - lastSync >= DAY_IN_MILLIS)
        {
            ...
            if (cursor.moveToFirst())
            {
                ...
                String contentText = String
                        .format(context.getString(R.string.format_notification), desc,
